      ...
      <header className="pb-8 border-b border-slate-100 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="max-w-2xl">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold tracking-tight text-slate-900 leading-tight">
            Sistem Dukungan{" "}
            <span className="text-slate-400 font-medium italic">Sobat.</span>
          </h1>
          <p className="mt-4 text-[15px] sm:text-[16px] text-slate-500 font-medium leading-relaxed">
            Tautkan kontak darurat dari lingkaran terdekatmu. Hubungi mereka
            dalam satu klik melalui pendampingan pesan yang telah terenskripsi.
          </p>
        </div>
      </header>
      ...
      <div className="grid lg:grid-cols-12 gap-10 lg:gap-16 items-start">
        ...
        <div className="lg:col-span-7 flex flex-col gap-10">
          <div className="flex items-center justify-between">
            ...
            <div
              className={`inline-flex items-center justify-center px-4 py-1.5 rounded-full text-[11px] font-bold ${buddies.length >= 2 ? "bg-emerald-50 text-emerald-600 border border-emerald-100" : "bg-slate-50 text-slate-500 border border-slate-100"}`}
            >
          ...
          {!buddies || buddies.length === 0 ? (
            <div className="bg-white rounded-[32px] p-10 sm:p-16 border border-slate-100 flex flex-col items-center text-center shadow-[0_8px_30px_rgba(0,0,0,0.02)] relative overflow-hidden group">
              ...
              <h3 className="text-2xl font-bold tracking-tight text-slate-900 mb-3 relative z-10">
                Lingkaranmu Masih Kosong
              </h3>
              <p className="text-[15px] text-slate-500 max-w-sm mb-10 leading-relaxed font-medium relative z-10">
                Dukungan dari orang terdekat adalah obat terbaik. Tambahkan
                hingga dua sobat kepercayaanmu sekarang.
              </p>
            ...
            <div className="space-y-8">
              {buddies.map((buddy) => (
                <div
                  key={buddy.id}
                  className="group bg-white rounded-[32px] border border-slate-100 shadow-[0_12px_40px_rgba(0,0,0,0.03)] hover:shadow-xl hover:border-slate-200 transition-all flex flex-col overflow-hidden"
                >
                  {/* Top: Identity */}
                  ...
                  <div className="bg-slate-50/30 p-8 sm:p-10 border-t border-slate-50">
                    <div className="flex flex-col gap-4">
                      <p className="text-[11px] font-bold text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                        <MessageCircle className="w-4 h-4" /> Quick Assistance
                        Templates
                      </p>
                      <div className="grid sm:grid-cols-2 gap-4">
                        {templates.map((tpl, i) => (
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <span className="text-[15px] font-bold text-slate-900 group-hover/btn:text-slate-600">
                                {tpl.title}
                              </span>
              ...
              {buddies.length > 0 && buddies.length < 2 && (
                <div className="bg-white p-8 sm:p-10 rounded-[32px] border border-dashed border-slate-200 shadow-sm">
                  <h3 className="text-[14px] font-bold text-slate-900 mb-8 flex items-center gap-3 uppercase tracking-widest opacity-60">
                    <UserPlus className="w-5 h-5" /> Tambah Kontak Mentor/Teman
                    Kedua
                  </h3>
          ...
          <div className="bg-slate-900 rounded-[32px] p-8 sm:p-10 text-white shadow-2xl relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <p className="text-[15px] text-slate-400 leading-relaxed font-medium mb-6">
                RuangSela{" "}
                <strong className="text-white">tidak mengotomatisasi</strong>{" "}
                pengiriman hasil tes stres Anda. Kami percaya bahwa keputusan
                untuk berbagi emosi adalah hak mutlak Anda.
              </p>
              <div className="bg-white/5 rounded-2xl p-5 border border-white/5 space-y-4">
                ...
                <p className="text-[14px] text-slate-300 leading-relaxed italic font-medium">
                  &quot;Kami hanya menyediakan akses tercepat saat kamu merasa
                  tidak sanggup lagi berpikir jernih.&quot;
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[32px] border border-slate-100 p-8 sm:p-10 shadow-sm">
            ...
            <ul className="space-y-8">
              <li className="flex gap-4 items-start">
                <div className="w-8 h-8 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center flex-shrink-0 text-[11px] font-bold text-slate-900 shadow-sm">
                  01
                </div>
                <div>
                  <h4 className="text-[15px] font-bold text-slate-900 leading-tight">
                    Pilih Pendengar Aktif
                  </h4>
                  <p className="text-[13px] text-slate-500 font-medium leading-relaxed mt-2 opacity-80">
                    Utamakan sosok yang tidak defensif dan mampu berempati tanpa
                    langsung menghakimi atau memberi solusi kilat.
                  </p>
                </div>
              </li>
              <li className="flex gap-4 items-start">
                ...
                <div>
                  ...
                  <p className="text-[13px] text-slate-500 font-medium leading-relaxed mt-2 opacity-80">
                    Beritahu mereka bahwa kamu memasukkan kontaknya sebagai
                    &quot;Sobat&quot; di RuangSela agar mereka tidak terkejut
                    saat menerima pesan.
                  </p>
                </div>
              </li>
              <li className="flex gap-4 items-start">
                <div className="w-8 h-8 rounded-xl bg-rose-50 border border-rose-100 flex items-center justify-center flex-shrink-0 text-[11px] font-bold text-rose-600 shadow-sm">
                  03
                </div>
                <div>
                  <h4 className="text-[15px] font-bold text-slate-900 leading-tight">
                  ...
                  <p className="text-[13px] text-slate-500 font-medium leading-relaxed mt-2 opacity-80">
                    Jika pikiran untuk menyakiti diri muncul, sobat saja tidak
                    cukup. Segera hubungi{" "}
                    <Link
                      href="/help"
                      className="text-rose-500 font-bold hover:underline"
                    >
                      Direktori Bantuan Ahli
                    </Link>
                    .
                  </p>
                </div>
