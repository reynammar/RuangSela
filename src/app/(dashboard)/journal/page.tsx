    ...
    <div className="max-w-4xl mx-auto w-full min-h-[90vh] flex flex-col pt-12 pb-32 sm:pb-40 px-6 sm:px-8 lg:px-12 animate-in fade-in duration-1000">
      <div className="mb-12 flex flex-col items-center">
        ...
        <h1 className="text-3xl sm:text-4xl font-bold tracking-tight text-slate-900 mb-4 text-center">
          Ruang Cerita{" "}
          <span className="text-slate-400 font-medium italic">Amanmu.</span>
        </h1>
        <p className="text-[15px] sm:text-16px text-slate-500 font-medium leading-relaxed max-w-lg text-center opacity-80">
          Tumpahkan beban pikiranmu secara privat. Enkripsi RuangSela memastikan
          setiap kata hanyalah milikmu sendiri.
        </p>
      </div>
      ...
      >
        ...
        <div className="bg-white rounded-[28px] border border-slate-200/80 shadow-[0_12px_48px_rgba(0,0,0,0.03)] overflow-hidden flex flex-col relative h-[65vh]">
          <div className="h-16 bg-white border-b border-slate-50 flex items-center justify-between px-6 sm:px-8 flex-shrink-0 z-10">
            <div className="flex items-center gap-1.5 opacity-40">
              <div className="w-2.5 h-2.5 rounded-full bg-slate-200"></div>
              <div className="w-2.5 h-2.5 rounded-full bg-slate-200"></div>
              <div className="w-2.5 h-2.5 rounded-full bg-slate-200"></div>
            </div>
            <span className="text-[12px] font-bold text-slate-400 tracking-tight uppercase tracking-widest">
              {currentDate}
            </span>
          ...
          <div className="flex flex-col sm:flex-row flex-1 min-h-0">
            <div className="w-full sm:w-[200px] border-b sm:border-b-0 sm:border-r border-slate-50 bg-slate-50/20 p-6 flex flex-col gap-6 overflow-y-auto sm:overflow-visible">
              <div>
                ...
                <div className="flex flex-row sm:flex-col flex-wrap sm:flex-nowrap gap-2 sm:gap-1.5">
                  {moods.map((m) => {
                    ...
                    return (
                      ...
                      >
                        <Circle
                          className={`w-2 h-2 mr-3 fill-current ${isSelected ? m.color : "text-slate-200"}`}
                        />
                        {m.label}
                      </button>
                    );
            ...
            <div className="flex-1 relative bg-white">
              <textarea
                id="content"
                name="content"
                className="w-full h-full bg-transparent border-0 resize-none text-[17px] sm:text-[18px] text-slate-800 leading-[1.8] font-medium placeholder:text-slate-300 focus:ring-0 outline-none px-8 pt-10 pb-8 sm:px-12 sm:pt-12 sm:pb-12"
              ...
              <div className="absolute bottom-8 right-8 pointer-events-none flex items-center gap-3">
                <div className="px-4 py-2 rounded-full bg-slate-50 border border-slate-100 shadow-sm text-[11px] font-bold text-slate-400 flex items-center gap-2">
                  <span
                    className={`w-1.5 h-1.5 rounded-full ${content.length >= 10 ? "bg-emerald-400" : "bg-slate-300 transition-colors duration-500"}`}
                  ></span>
