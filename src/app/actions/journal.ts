...
async function generateAIInsights(content: string): Promise<AIInsightResult> {
  ...
  try {
    ...
    return {
      summary: "Terima kasih sudah berbagi. Maaf, saat ini modul analisis AI sedang beristirahat sebentar, namun ceritamu sudah tersimpan aman.",
      emotions: ["Telah Berbagi"],
      triggers: ["-"],
      coping_suggestions: ["Tarik napas dalam-dalam.", "Coba baca kembali tulisanmu besok."],
...
export async function submitJournalEntry(formData: FormData) {
  ...
  if (!content || content.trim().length < 10) {
    return { success: false, error: "Jurnal terlalu singkat. Ceritakan sedikit lebih banyak." }
  }
